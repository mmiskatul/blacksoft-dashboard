...
import { useServiceCards, addServiceCard, updateServiceCard, deleteServiceCard, type ServiceCard } from '../../utils/servicesStore';
...
export default function WhatWeDoPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedId) {
        await updateServiceCard(selectedId, draft);
      } else {
        await addServiceCard(draft);
      }
      closeEditor();
    } catch {
  ...
  const handleToggle = async (card: ServiceCard) => {
    try {
      await updateServiceCard(card.id, { enabled: !card.enabled });
    } catch {
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px', gap: '16px', flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: '14px', color: 'var(--text-muted)' }}>
            Configure and manage the primary service cards showcased on the homepage landing page.
          </p>
        </div>
        <button
          type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '32px' }}>
        {[
          { label: 'Total cards', val: cards.length, color: '#818cf8' },
          { label: 'Visible on landing page', val: visibleCount, color: '#34d399' },
          { label: 'Hidden cards', val: hiddenCount, color: '#f87171' },
        ].map((stat, i) => (
          <div
            key={i}
            style={{
          ...
          {hydrated && cards.length === 0 && (
            <div style={{
              ...
              <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '14px' }}>
                No service cards created yet. Click &quot;Add Service Card&quot; to begin.
              </p>
            </div>
          )}
            ...
            {cards.map((card) => {
              ...
              return (
                <div
                  ...
                  style={{
                    background: 'var(--bg-card-high)',
                    border: `1.5px solid ${isActive ? '#4f46e5' : 'var(--border-card)'}`,
                    borderRadius: '16px',
                    padding: '24px',
                    display: 'flex',
              ...
              <form onSubmit={handleSave} style={{ display: 'grid', gap: '16px' }}>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. AI Workflow Systems"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Describe this service or cognitive capability…"
                  />
                </div>
